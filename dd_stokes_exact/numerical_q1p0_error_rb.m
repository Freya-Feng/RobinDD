function [uele_h1,uele_l2,pele_l2]=numerical_q1p0_error_rb(xy,ev,xst,handle_solvelocity,handle_p,handle_velocity_gradient,viscosity)
...
%         pele_l2           element contribution for pressure error in L2 norm
...
% evaluate derivatives etc
         ...
         vh=zeros(nel,1);
         for i=1:4
             duhdx=duhdx+u_v(:,i).*dphidx(:,i).*invjac;
             duhdy=duhdy+u_v(:,i).*dphidy(:,i).*invjac;
             dvhdx=dvhdx+v_v(:,i).*dphidx(:,i).*invjac;
             dvhdy=dvhdy+v_v(:,i).*dphidy(:,i).*invjac;
             uh=uh+u_v(:,i).*phi(:,i);
             vh=vh+v_v(:,i).*phi(:,i);
         end
         [xigpt,yigpt]=qonemap(sigpt,tigpt,xy,ev);   
         [dudx,dudy,dvdx,dvdy]=handle_velocity_gradient(xigpt,yigpt);
         dudx=dudx/viscosity;dudy=dudy/viscosity;dvdx=dvdx/viscosity;dvdy=dvdy/viscosity;
         p=handle_p(xigpt,yigpt);
         [u,v]=handle_solvelocity(xigpt,yigpt);
         u=u/viscosity;v=v/viscosity;
